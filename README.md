...
- **이미지에서 가져오기**: 파일 선택 또는 문제 textarea에 Ctrl+V로 이미지를 붙여넣으면
  이미지 속 문제 텍스트와 수식을 `$...$` LaTeX 형식으로 추출합니다. 여러 장을 연달아
  붙여넣거나 한 번에 여러 파일을 선택하면 큐에 쌓여 순서대로 처리되고, 결과가 모두 같은
  textarea에 누적됩니다 — 문제마다 따로 변환할 필요 없이 마지막에 한 번만 "hwpx로 변환"을
  누르면 됩니다.
- **AI로 생성·변형**: 자연어로 요청("이차함수 최댓값 문제 2개 만들어줘", "위 문제를
  서술형으로 바꿔줘" 등)하면 현재 textarea 내용을 참고(변형 대상 또는 스타일 참고)해
  문제+해설을 작성해 같은 textarea에 추가합니다. 여러 번 요청해 여러 문제를 쌓은 뒤
  한 번에 변환하면 hwpx 한 파일에 모두 담깁니다.

두 기능 모두 결과를 자동으로 변환하지 않고 textarea에 채우기만 하므로, AI가 만든 내용의
정답과 해설을 사람이 확인한 뒤 변환 버튼을 눌러야 합니다.
