...
- **자동 저장 켬/끔** (머리글 "API 키 입력" 옆 토글, 도형 생성기와 같음): 토글 하나가
  API 키와 작업을 함께 다룹니다.
  - **켬**: 키와 작업을 이 브라우저(localStorage)에 저장합니다. 새로 고침해도 키 연동이
    남고, 다음에 열면 지난 작업을 따로(`hwpeq_work_prev`) 옮겨 두고 "이어서 하기 / 새로
    시작"을 묻습니다. 묻지 않고 같은 자리에 새 작업을 저장하면, 대답하기 전에 박스 하나만
    만들어도 지난 작업이 덮이기 때문입니다. 켤 때 공용 컴퓨터 주의 풍선이 뜹니다.
  - **끔**: 아무것도 남기지 않습니다. 키는 이번 창의 메모리에만(새로 고침하면 연동이 끊김),
    작업은 저장하지 않고 다음에 처음부터 시작합니다. 끄는 순간 브라우저에 남은 키 · 작업 ·
    지난 작업을 모두 지웁니다.
  - 기본값(`apiKeyPersist` 가 없을 때): 이미 키나 작업을 저장해 쓰던 사람은 켬(갑자기 끊기지
    않게), 처음 쓰는 사람은 끔.
  - 키는 모두 `keyGet` / `keySet` / `keyDel` 로만 읽고 씁니다(`public/index.html`).
...
키는 서버에 저장되지 않습니다. 머리글의 **자동 저장**을 켜면 브라우저 localStorage 에
회사별로 저장되고(`geminiApiKey`, `openaiApiKey`, `anthropicApiKey`), 끄면 이번 창의
메모리에만 둡니다. 저장 이름은 **도형 생성기와 같아서**, 나중에 두 앱을 같은 주소에
올리면 키가 그대로 공유됩니다.
